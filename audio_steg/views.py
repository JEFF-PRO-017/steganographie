...
def index(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # to get type file data
            type_file = typeFiles(file)
            print(type_file)
            print(type_file)
            print(type_file)
            print(type_file)

            context = {'form': form, 'valEncode': valEncode, 'choice': choice, 'textField': type_file}
            return render(request, 'audio_steg/index.html', context)

    # Si on a pas  de data on genere le form
    else:
        form = FilesForm()
    return render(request, 'audio_steg/index.html', {'form': form})
